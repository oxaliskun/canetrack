...
<div class="print-container">
  ...
  <div class="grid-2">
    <div class="info-group">
      ...
      <div class="info-group">
        <h4>Truck & Driver</h4>
        <div class="field"><div class="field-label">Plate Number</div><div class="field-value">${ticket.truck.plateNumber}</div></div>
        ${ticket.truck.driverName ? `<div class="field"><div class="field-label">Driver</div><div class="field-value">${ticket.truck.driverName}</div></div>` : ''}
        ${ticket.truck.driverContact ? `<div class="field"><div class="field-label">Contact</div><div class="field-value">${ticket.truck.driverContact}</div></div>` : ''}
      </div>
      ` : ticket.truckNumber ? `
      <div class="info-group">
        <h4>Truck</h4>
        <div class="field"><div class="field-label">Truck No.</div><div class="field-value">${ticket.truckNumber}</div></div>
      </div>
      ` : ''}
      <div class="info-group">
        <h4>Delivery & Cane Details</h4>
        ${(ticket.truck || ticket.truckNumber) ? `<div class="field"><div class="field-label">Truck No.</div><div class="field-value">${ticket.truck?.plateNumber || ticket.truckNumber}</div></div>` : ''}
                ...
                <div className="p-6 sm:p-8 space-y-8 overflow-y-auto max-h-[calc(100vh-12rem)]">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      {(ticket.truck || ticket.truckNumber) && (
                        <div className={`p-5 rounded-2xl border relative overflow-hidden ${isDark ? 'border-slate-700 bg-slate-800/50' : 'border-slate-100 bg-slate-50/50'}`}>
                          <div className="flex items-center gap-2 mb-4">
                            <div className={`p-2 rounded-xl ${isDark ? 'bg-amber-900/30 text-amber-400' : 'bg-amber-100 text-amber-600'}`}>
                              <Truck className="w-4 h-4" />
                            </div>
                            <span className={`text-xs font-extrabold uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Truck & Driver</span>
                          </div>
                          <p className={`font-bold text-base ${isDark ? 'text-white' : 'text-slate-900'}`}>{ticket.truck?.plateNumber || ticket.truckNumber || '-'}</p>
                          {ticket.truck?.driverName && (
                            <div className="flex items-center gap-1.5 mt-2 text-xs">
                              <User className={`w-3 h-3 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                              <span className={isDark ? 'text-slate-400' : 'text-slate-500'}>{ticket.truck.driverName}</span>
                            </div>
                          )}
                          {ticket.truck?.driverContact && (
                            <div className="flex items-center gap-1.5 mt-1 text-xs">
                              <Phone className={`w-3 h-3 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                              <span className={isDark ? 'text-slate-400' : 'text-slate-500'}>{ticket.truck.driverContact}</span>
                            </div>
                          )}
                        </div>
                      )}
                  ...
                  {(ticket.truckNumber || ticket.deliveryDate || ticket.authorizedSignatory || ticket.caneVariety || ticket.loadRemarks || ticket.unloadingType) && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {(ticket.truck || ticket.truckNumber) && <DetailRow label="Truck No." value={ticket.truck?.plateNumber || ticket.truckNumber || '-'} icon={Truck} />}
