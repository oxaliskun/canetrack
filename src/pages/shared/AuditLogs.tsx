...
export function AuditLogs() {
  ...
  useEffect(() => {
    ...
    setPage(1);
    fetchLogs(1);
  }, [filterAction, dateFrom, dateTo, searchTerm]);
  ...
  const selectClass = `px-4 py-3 border rounded-xl text-sm font-bold outline-none focus:ring-2 cursor-pointer ${isDark ? 'bg-slate-800 border-slate-700 text-white focus:ring-emerald-500/20' : 'bg-slate-50 border-slate-200 text-slate-700 focus:ring-emerald-500/20'}`;
  const inputClass = `px-4 py-3 border rounded-xl text-sm font-bold outline-none focus:ring-2 ${isDark ? 'bg-slate-800 border-slate-700 text-white focus:ring-emerald-500/20' : 'bg-slate-50 border-slate-200 text-slate-700 focus:ring-emerald-500/20'}`;
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col mb-4 relative">
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-3 border w-max ${isDark ? 'bg-slate-800 text-slate-300 border-slate-700' : 'bg-slate-200 text-slate-700 border-slate-300'}`}>
          <History className="w-3.5 h-3.5" /> My Activity
        </div>
        ...
        <p className={`mt-1 text-sm sm:text-base font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Review your recent actions and system activity.</p>
      </div>
            ...
            <tbody className={`divide-y font-medium ${isDark ? 'divide-slate-700 text-slate-300' : 'divide-slate-100 text-slate-700'}`}>
              {logs.map((log: any) => (
                <tr key={log.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td data-label="Timestamp" className={`px-4 sm:px-6 py-4 sm:py-5 text-sm font-mono ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{formatDate(log.timestamp)}</td>
                </tr>
              ))}
              {logs.length === 0 && (
                <tr>
                  <td colSpan={3} className={`px-4 sm:px-6 py-12 sm:py-20 text-center text-base sm:text-lg ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
