              ...
              {filtered.map((t: any) => (
                <tr key={t.id} className={`transition-colors ${isDark ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td data-label="Actions" className="px-4 sm:px-6 py-4 sm:py-5 text-right">
                    <button onClick={() => setSelectedTicketId(t.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-extrabold uppercase tracking-widest border transition-all min-h-[36px] ml-auto ${isDark ? 'text-blue-400 border-blue-800 hover:bg-blue-950/50' : 'text-blue-600 border-blue-200 hover:bg-blue-50'}`}>
