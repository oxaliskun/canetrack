...
import { Truck, Plus, Edit3, Archive, X, Search, Save, User, Phone } from 'lucide-react';
...
const typeOptions = ['14ft', '18ft', '20ft'];
const emptyForm: TruckForm = { plateNumber: '', driverName: '', driverContact: '', compatibleTypes: ['18ft'] };
...
export function Trucks() {
  ...
  const openEdit = (truck: TruckData) => {
    ...
    setForm({
      ...
      compatibleTypes: truck.compatibleTypes.split(','),
    });
    setModalOpen(true);
  };
  ...
  const toggleType = (t: string) => {
    setForm(prev => ({
      ...prev,
      compatibleTypes: prev.compatibleTypes.includes(t)
        ? prev.compatibleTypes.filter(x => x !== t)
        : [...prev.compatibleTypes, t]
    }));
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!plate || !form.driverName.trim()) {
      toast.error('Plate number and driver name are required');
      return;
    }
    if (form.compatibleTypes.length === 0) {
      toast.error('Select at least one compatible trailer type');
      return;
    }
    const compatibleTypes = form.compatibleTypes.join(',');
    try {
      if (editTruck) {
        await api.patch(`/trucks/${editTruck.id}`, { plateNumber: plate, driverName: form.driverName.trim(), driverContact: form.driverContact || null, compatibleTypes });
        toast.success('Truck updated successfully');
      } else {
        await api.post('/trucks', { plateNumber: plate, driverName: form.driverName.trim(), driverContact: form.driverContact || null, compatibleTypes });
  ...
  const handleArchive = async (truck: TruckData) => {
    try {
      await api.patch(`/trucks/${truck.id}`, { isArchived: !truck.isArchived });
    ...
    <div className="space-y-8 max-w-7xl mx-auto relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4 relative">
        <div>
          ...
          <p className={`mt-1 text-sm sm:text-base font-medium ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>Manage your trucks and driver information.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 mb-6 sm:mb-8">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border relative overflow-hidden ${isDark ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
          ...
          <div className="flex items-center justify-between mb-2 sm:mb-3 relative z-10">
           ...
           <p className={`text-2xl sm:text-3xl font-black tracking-tight relative z-10 truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{trucks.length}</p>
         ...
         <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border relative overflow-hidden ${isDark ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
           ...
           <div className="flex items-center justify-between mb-2 sm:mb-3 relative z-10">
             <p className={`font-extrabold text-[10px] sm:text-[11px] uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total Drivers</p>
             <div className={`p-2 sm:p-2.5 rounded-xl bg-blue-100 text-blue-700`}><User className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></div>
           </div>
           <p className={`text-2xl sm:text-3xl font-black tracking-tight relative z-10 truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{activeTrucks.length}</p>
           <p className={`text-xs sm:text-sm mt-1 ml-0.5 font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>registered drivers</p>
         </motion.div>
         ...
         <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl border relative overflow-hidden ${isDark ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
           ...
           <div className="flex items-center justify-between mb-2 sm:mb-3 relative z-10">
             <p className={`font-extrabold text-[10px] sm:text-[11px] uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Compatibility</p>
             <div className={`p-2 sm:p-2.5 rounded-xl bg-purple-100 text-purple-700`}><Truck className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></div>
           </div>
           <p className={`text-2xl sm:text-3xl font-black tracking-tight relative z-10 truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{activeTrucks.reduce((s, t) => s + t.compatibleTypes.split(',').length, 0)}</p>
           <p className={`text-xs sm:text-sm mt-1 ml-0.5 font-medium ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>total type compatibilities</p>
         </motion.div>
      </div>
                ...
                <div className="p-4 sm:p-6 flex-1 flex flex-col relative">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      <h3 className={`text-base sm:text-lg font-black tracking-tight truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{truck.plateNumber}</h3>
                      <p className={`text-sm font-medium mt-0.5 truncate flex items-center gap-1.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        <User className="w-3.5 h-3.5 shrink-0" /> {truck.driverName}
                      </p>
                      {truck.isArchived && (
                  ...
                  <div className="space-y-3 mb-6 flex-1">
                    {truck.driverContact && (
                      <div className="flex items-center gap-2.5">
                        <Phone className={`w-4 h-4 shrink-0 ${isDark ? 'text-slate-500' : 'text-slate-400'}`} />
                        <span className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>{truck.driverContact}</span>
                      </div>
                    )}
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`text-[10px] font-extrabold uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Compatible:</span>
                      {truck.compatibleTypes.split(',').map(type => (
                        <span key={type} className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold ${isDark ? 'bg-emerald-900/30 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>
                          {type}
                        </span>
                      ))}
                    </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
              ...
              <div className="p-4 sm:p-6 lg:p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  ...
                  <div>
                    <label className={`block text-[11px] font-extrabold uppercase tracking-widest mb-2 ml-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Driver's Name *</label>
                  ...
                  <div>
                    <label className={`block text-[11px] font-extrabold uppercase tracking-widest mb-2 ml-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Driver's Contact Number</label>
                    <div className="relative">
                      <input value={form.driverContact} onChange={e => setForm({...form, driverContact: e.target.value})}
                        className={`w-full pl-9 sm:pl-10 pr-4 sm:pr-5 py-3 sm:py-3.5 border rounded-xl sm:rounded-2xl focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none font-medium text-sm shadow-sm min-h-[44px] ${isDark ? 'bg-slate-800 border-slate-700 text-white placeholder:text-slate-500' : 'bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400'}`}
                        placeholder="+63 912 345 6789" />
                      <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    </div>
                  </div>
                  <div>
                    <label className={`block text-[11px] font-extrabold uppercase tracking-widest mb-2 ml-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Compatible Bagon Types *</label>
                    <div className="flex gap-3">
                      {typeOptions.map(t => (
                        <label key={t} className={`flex items-center gap-2 px-4 py-3 border rounded-xl sm:rounded-2xl cursor-pointer transition-all text-sm font-bold min-h-[44px] flex-1 justify-center ${form.compatibleTypes.includes(t)
                          ? (isDark ? 'bg-emerald-900/50 border-emerald-600 text-emerald-400' : 'bg-emerald-50 border-emerald-500 text-emerald-700')
                          : (isDark ? 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-slate-100')}`}>
                          <input type="checkbox" checked={form.compatibleTypes.includes(t)} onChange={() => toggleType(t)} className="sr-only" />
                          {t}
                        </label>
                      ))}
                    </div>
